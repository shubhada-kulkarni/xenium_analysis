...
# Do not use either boundary or interior stain and only use DAPI for nuclear expansion with expansion distance 0 (recommended by Lorena)
/prj/XeniumProbeDesign/analyser_installation/xeniumranger-xenium2.0/xeniumranger resegment --xenium-bundle $in_folder --id $out_id --boundary-stain=disable --interior-stain=disable --localcores=128 --localmem=256 --expansion-distance=0
>>>>>>> Stashed changes
